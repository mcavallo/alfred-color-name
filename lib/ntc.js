...
function toVariableName(str) {
  return '$color-' + str.toLowerCase()
    .replace(/[^a-z]/, ' ')
    .replace(/\s+/, '-');
}

function Color(color, queriedColorRgbParts, isExact) {
  this.hex = '#' + color[0];
  this.rgb = 'rgb(' + color[2] + ', ' + color[3] + ', ' + color[4] + ')';
  this.rgbParts = [color[2], color[3], color[4]];
  this.queriedRgbParts = queriedColorRgbParts;
  this.name = color[1];
  this.variableName = toVariableName(color[1]);
  this.sassDefinition = this.variableName + ': ' + this.hex + ';';
  this.matchType = !!isExact ? 'Exact' : 'Close match';
}
...
var ntc = {
  ...
  name: function(color) {
    ...
    for (var i = 0; i < ntc.names.length; i++) {
      if (color == "#" + ntc.names[i][0]) {
        return (new Color(ntc.names[i], rgb, true));
      }
    ...
    return (cl < 0 ? false : new Color(ntc.names[cl], rgb, false));
  },
  ...
  hexRegex: /^([a-f0-9]{6}|[a-f0-9]{3})\b$/i,
